...
void allocate_csr_row(csr *csr_mat, int num_rows){
	...
	cudaMallocManaged(&csr_mat->d_rowIndices, (num_rows+1) * sizeof(int));
...
void allocate_csr_col_val(csr *csr_mat, int nnz){
	...
	cudaMallocManaged(&csr_mat->d_values, nnz*sizeof(double));
	cudaMallocManaged(&csr_mat->d_colIndices, nnz * sizeof(int));
...
void multiply_csr(csr *A, csr *B, csr *C, int m, int n, int k, cusparseHandle_t context,cusparseMatDescr_t descr ){
	...
	allocate_csr_row(C, m);
	//cudaMallocManaged(&C->d_rowIndices, (m+1) * sizeof(int));
...
__global__ void unbalanced_dgmm(double *diag, double *mat, int m, int n, int k){
...
__global__ void deepwalk_filter(double *A, int size, double window){
	int id = blockIdx.x * blockDim.x + threadIdx.x;
...
int main ( int argc, char **argv ){
	...
	*/
	...
	cudaMallocManaged(&adj_device_dense, 
			g.size * g.size * sizeof(double)); 	
	cudaMallocManaged(&degree_device_dense, 
			g.size * sizeof(double)); 

	/* Step 3: Copy dense matrix from host to device */
	log("Copying dense matrix from host to device");	
	std::memcpy(adj_device_dense, g.degree, g.size * g.size * sizeof(double));
	std::memcpy(degree_device_dense, g.degree, g.size * sizeof(double));

//	/*REMOVE*/
//	//std::sort(g.degree1D, g.degree1D + g.size);
//	//cudaMemcpy(degree_host_dense, degree_device_dense, g.size * sizeof(double), cudaMemcpyDeviceToHost);
//
...
//	/*REMOVE*/
//	//cudaMemcpy(g.degree1D, degree_device_dense, g.size * sizeof(double), cudaMemcpyDeviceToHost);
//	//std::sort(g.degree1D, g.degree1D + g.size);
//
...
//
	...
	cudaMallocManaged(&X_temp_device, g.size * g.size * sizeof(double));
	...
	double *X_device;
//	double *X_host;
	cudaMallocManaged(&X_device, g.size * g.size * sizeof(double));
	...
	/*REMOVE*/
//	//X_host = (double *) malloc(sizeof(double) * g.size * g.size);
//	//cudaMemcpy(X_host, X_device, g.size * g.size * sizeof(double), cudaMemcpyDeviceToHost);
...
//
	...
	double *e_device;
	double *EV_device;
	cudaMallocManaged(&e_device, g.size * sizeof(double));
	cudaMallocManaged(&EV_device, g.size * rank * sizeof(double));
	...
	cudaMallocManaged(&d_work, sizeof(double) * lwork);
	...
	cusolverDnDsyevdx(
	    ...
	    devInfo);
	cudaDeviceSynchronize();
	memcpy(EV_device, X_device, g.size * rank * sizeof(double));
//	log("Computed eigen");
//
//	/*REMOVE*/
//	double *e_host;
//	e_host = (double *)malloc(g.size * sizeof(double));
//	////cudaMemcpy(e_host, e_device, g.size * sizeof(double), cudaMemcpyDeviceToHost);
//	//double *EV_host;
//	//EV_host = (double *) malloc(sizeof(double) * g.size * rank);
//	//gpuErrchk(cudaMemcpy(EV_host, X_device, g.size * rank * sizeof(double), cudaMemcpyDeviceToHost));
//	//
...
//
//	double *D_rt_invU_device;
//	cudaMallocManaged(&D_rt_invU_device, g.size * rank * sizeof(double));
//	cublasDdgmm(cublas_handle, CUBLAS_SIDE_LEFT,
//		g.size, rank,
//		EV_device, g.size, 
//		degree_device_dense, 1,
...
//	cudaDeviceSynchronize();
//
//	cudaMemcpy(e_host, e_device, rank * sizeof(double), cudaMemcpyDeviceToHost);
//	
...
//
//	///*REMOVE*/
//	//cudaMemcpy(EV_host, EV_device, g.size * rank * sizeof(double), cudaMemcpyDeviceToHost);
//	//std::cout<<std::endl;
...
//
//	double *M_device;
//	cudaMallocManaged(&M_device, g.size * g.size * sizeof(double));
//
//	double alf = 1.00;
//	double beta = 0.00;
//
...
//	//std::cout<<std::endl;
//	//double *M_host;
//	//M_host = (double *) malloc(sizeof(double) * g.size * g.size);
//	//cudaMemcpy(M_host, M_device, g.size * g.size * sizeof(double), cudaMemcpyDeviceToHost);
//	//for(int i=0;i<g.size * g.size;i++){
//	//	std::cout<<"M "<<i<<":"<<M_host[i]<<std::endl;
//	//}
//
//	
//	double val = ((double) g.volume) / ((double) b);
...
//
//	cudaMallocManaged(&M_cap.d_nnzPerVector, 
//			g.size * sizeof(int));
...
//
//	cudaMallocManaged(&U_device, g.size * dimension * sizeof(double));
//	cudaMallocManaged(&E_device, g.size * dimension * sizeof(double));
//	cudaMallocManaged(&Si_device, dimension * sizeof(double));
//
