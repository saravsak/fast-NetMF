...
int main (int argc, char *argv[] ){
	...
	*/
	...
	/* Tiling housekeeping */
	int tile_size = 8;
	int num_tiles = (g.size) / (tile_size);
	...
	/* CUDA housekeeping */
	begin = Clock::now();
	float num_threads = tile_size;
	dim3 threads(num_threads);
	dim3 grids((int)ceil((float)(tile_size*tile_size)/num_threads));
	...
	DT *degree_d[num_streams];
	
	DT *degree_h;
	gpuErrchk(cudaHostAlloc(&degree_h, g.size * sizeof(DT), cudaHostAllocDefault));
	memcpy(degree_h, g.degree1D, g.size * sizeof(DT));
		
	for(int i=0;i<num_streams;i++){
		gpuErrchk(cudaMalloc(&tile_d[i], 
					tile_size * tile_size * sizeof(DT)));
		gpuErrchk(cudaMalloc(&degree_d[i], 
					tile_size * sizeof(DT)));
	}
	...
	for(int i=0;i<num_tiles;i++){	
		gpuErrchk(cudaHostAlloc(&tile[i], 
				tile_size*tile_size*sizeof(DT), 
	...
	while(idx<num_tiles){
		...
		for(int i=0;i<num_streams;i++){
			...
			std::cout<<"Working on tile: "<<idx<<std::endl;
			
			make_tile(g.adj, tile[idx], g.size, g.size, tile_size, tile_size, idx, idx);
	
			gpuErrchk(cudaMemcpyAsync(tile_d[i], 
						tile[idx], 
						tile_size * tile_size * sizeof(DT), 
						cudaMemcpyHostToDevice, 
						streams[i]));
	
			gpuErrchk(cudaMemcpyAsync(degree_d[i], 
						degree_h + (idx * tile_size), 
						tile_size * sizeof(DT), 
						cudaMemcpyHostToDevice, 
						streams[i]));
	
			preprocess_laplacian<<<grids, threads, 0, streams[i]>>>(
					tile_d[i], 
					degree_d[i], 
					tile_size);
		...
		for(int i=0;i<num_streams;i++){
			if(ret_idx>=num_tiles){
				break;
			}
			cudaStreamSynchronize(streams[i]);
			
			gpuErrchk(cudaMemcpyAsync(tile[ret_idx], 
						tile_d[i], 
						tile_size * tile_size * sizeof(DT), 
						cudaMemcpyDeviceToHost, 
						streams[i]));
	
			gpuErrchk(cudaMemcpyAsync(degree_h + (ret_idx * tile_size), 
						degree_d[i], 
						tile_size * sizeof(DT), 
						cudaMemcpyDeviceToHost, 
						streams[i]));
	...
	if(VERIFY){
		for(int i=0;i<g.size;i++){
			if(g.adj[i * g.size + i] != 0 && degree_h[i] != 1){
				std::cout<<"Preprocess laplacian failed: "<<i<<std::endl;
				exit(0);
			}
			if(1/sqrt(g.degree1D[i]) != degree_h[i]){
				std::cout<<"Preprocess laplacian failed on cond2: "<<i<<std::endl;
				exit(0);
			}
		}
		std::cout<<"Verified Preprocess laplacian"<<std::endl;
	}

//	begin = Clock::now();
	log("Preprocessed Laplacian");
	...
	*/
	...
	log("Computing X' = D' * A");
	DT *X_temp_h;
	X_temp_h = (DT *) malloc(g.size * g.size * sizeof(DT));
