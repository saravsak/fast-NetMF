...
MatrixXf netmf(Eigen::SparseMatrix<float> &A, const string &graph_size, unsigned long long int rank, unsigned long long int convergence_controller_by_user, int negative_sampling, unsigned long long int dim, cpu_timer &timer )
{
	...
	// Vol and D matrix involves the sum of weights and not the sum of degrees. 
	...
	#ifdef PRINT_EXECUTION_TIME	
	print_timer("d_Approximated M" , timer);
	#endif /* PRINT_EXECUTION_TIME */
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		unsigned long long int convergence_controller;
		...
		("convergence_controller,cc" ,program_options::value<unsigned long long int>(&convergence_controller)->default_value(0) , "convergence controller parameter for eigenvalue solver..is usually > 2*rank but less equal than number of nodes")
		...
		std::cout << "convergence_controller_by_user (0 will convert to 2.1*rank) : " << convergence_controller << std::endl;
		...
		#ifdef PRINT_EXECUTION_TIME		
		print_timer("a_Graph Loaded from file", timer);
		#endif /* PRINT_EXECUTION_TIME */
		...
		network_embedding << VectorXf::LinSpaced(num_nodes,0,num_nodes-1), netmf(A, graph_size, rank, convergence_controller, negative_sampling, dim, timer);
		...
		#ifdef PRINT_EXECUTION_TIME	
		print_timer("f_Embedding Ready" , timer);
		#endif /* PRINT_EXECUTION_TIME */
		...
		#ifdef PRINT_EXECUTION_TIME	
		print_timer("g_Embedding Written to file", timer);
		#endif /* PRINT_EXECUTION_TIME */
